...
export default function PanelTutorado() {
    ...
    useEffect(() => {
        if (user === undefined || user === null) {
            serUser(JSON.parse(localStorage.getItem("user")))
            history.go(0)
        } else {
            ...
            <Container fluid className="text-center py-3 " style={{ backgroundColor: "rgba(230,230,230,0.7)" }}>
                <Row className="justify-content-center">
                    ...
                    <Col className="col-sm-auto col-md-auto col-lg-auto">
                        ...
                        <center>Horas realizadas de tutorias: {(datosTabla.horasOnline + datosTabla.horasPresencial)}</center>
                    </Col>
                </Row>
            </Container>
            :
            <Container fluid className="text-center py-3 " style={{ backgroundColor: "#E6E6E6" }}>
                <Row>
                    <Col className="col">
                        <hr />
                        <label className="alert alert-danger" role="alert">Problemas al cargar informacion de alumno</label>
                        <hr />
                    </Col>
                </Row>
            </Container>}
...
function CambioTutor() {
    ...
    const [user2, serUser2] = useState({ id: JSON.parse(localStorage.getItem("user")).id, motivo: "" });
...
function CambioFoto() {
    ...
    const Toast = Swal.mixin({ toast: true, position: 'bottom-end', showConfirmButton: false, timer: 750 })
    const [show, setShow] = useState(false);
    const [user2, serUser2] = useState({ id: JSON.parse(localStorage.getItem("user")).id, documento: "", type: "foto" });
    ...
    async function enviarFoto() {
        try {
            ...
            if (resultUser !== null) {
                await Toast.fire({ icon: 'success', title: resultUser })
                let resultFoto = await apiCall({ url: "datos/alumno/foto", body: {id: JSON.parse(localStorage.getItem("user")).id} });
                console.log(resultFoto)
